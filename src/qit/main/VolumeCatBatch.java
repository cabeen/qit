...
public class VolumeCatBatch implements CliMain
{
    public static void main(String[] args)
    {
        new VolumeCatBatch().run(Lists.newArrayList(args));
    }
    ...
    public void run(List<String> args)
    {
        try
        {
            ...
            String doc = "concatenate volumes in batch mode";
            ...
            cli.withOption(new CliOption().asOutput().asOptional().withName("output").withArg("<Volume>").withDoc("specify the output concatenated volume"));
            ...
            for (int i = 0; i < inputFns.size(); i++)
            {
                ...
                if (out == null)
                {
                    out = volume.proto(inputFns.size());
                }

                for (Sample sample : out.getSampling())
                {
                    out.set(sample, i, volume.get(sample, 0));
                }
            }

            Logging.info("writing output: " + outputFn);
            out.write(outputFn);

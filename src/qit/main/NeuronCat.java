...
public class NeuronCat implements CliMain
{
    public static void main(String[] args)
    {
        new NeuronCat().run(Lists.newArrayList(args));
    }
    ...
    public void run(List<String> args)
    {
        try
        {
            ...
            String doc = "Concatenate neuron files";

            CliSpecification cli = new CliSpecification();
            cli.withName(this.getClass().getSimpleName());
            cli.withDoc(doc);
            cli.withOption(new CliOption().asInput().withName("input").withArg("<Neuron(s)>").withDoc("specify an input neuron files").withNoMax());
            cli.withOption(new CliOption().asOutput().withName("output").withArg("<Neuron>").withDoc("specify the neuron file"));
            ...
            for (String fn : input)
            {
                try
                {
                    ...
                    neuron.add(Neuron.read(fn));
                }
                catch (Exception e)
                {
                    Logging.info("WARNING: failed to add neuron, skipping it");
                }
            }

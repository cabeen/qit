...
public class MapCatNarrow implements CliMain
{
    public static void main(String[] args)
    {
        new MapCatNarrow().run(Lists.newArrayList(args));
    }
    ...
    public void run(List<String> args)
    {
        try
        {
            ...
            String doc = "concatenate map files into columns of a table (more efficient than MapCat)";
            ...
            cli.withOption(new CliOption().asParameter().asOptional().withName("output-name").withArg("<String>").withDoc("specify the output map name field").withDefault("name"));
            cli.withOption(new CliOption().asParameter().asOptional().withName("output-value").withArg("<String>").withDoc("specify the output map value field").withDefault("value"));
            ...
            output.withField(nameOutputField);
            output.withField(valueOutputField);
            for (String field : vars.keySet())
            {
                output.withField(field);
            }
            ...
            for (Map<String, String> group : ComboUtils.groups(vars, product))
            {
                ...
                try
                {
                    ...
                    Table map = Table.read(fn);

                    int nameIndex = map.getFieldIndex(nameInputField);
                    int valueIndex = map.getFieldIndex(valueInputField);

                    for (String[] entry : map)
                    {
                        Record row = new Record();
                        row.with(nameOutputField, entry[nameIndex]);
                        row.with(valueOutputField, entry[valueIndex]);
                        for (String var : group.keySet())
                        {
                            row.with(var, group.get(var));
                        }
            ...
            if (output.getNumRecords() > 0)
            {
                Logging.info("writing: " + outputFn);
                output.write(outputFn);
            }
            else
            {

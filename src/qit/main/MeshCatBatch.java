...
public class MeshCatBatch implements CliMain
{
    public static void main(String[] args)
    {
        new MeshCatBatch().run(Lists.newArrayList(args));
    }
    ...
    public void run(List<String> args)
    {
        try
        {
            ...
            String doc = "Concatenate mesh files";

            CliSpecification cli = new CliSpecification();
            cli.withName(this.getClass().getSimpleName());
            cli.withDoc(doc);
            cli.withOption(new CliOption().asInput().withName("input").withArg("<File(s)>").withDoc("specify an input meshes").withNoMax());
            ...
            for (String name : fns.keySet())
            {
                ...
                try
                {
                    ...
                    if (mesh.vattr.size() == 0)
                    {
                        Logging.info("warning: skipping empty mesh");
                        continue;
                    }

                    if (label)
                    {
                        mesh.vattr.add(attr, VectSource.create1D(0));
                        MeshUtils.setAll(mesh, attr, VectSource.create1D(idx));
                ...
                {
                    Logging.info("warning: skipping " + fn);
                }
            }
            ...
            {
                Logging.info("warning: no output found");
            }

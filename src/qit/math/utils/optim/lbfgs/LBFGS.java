 ...
 * Broyden-Fletcher-Goldfarb-Shanno (LBFGS) algorithm for large-scaleCamera
 ...
 * 	,       large scaleCamera optimization methods'' Mathematical Programming 45
 * 	,       (1989), pp. 503-528.
...
public class LBFGS
{
    ...
    {
        private static final long serialVersionUID = -5323733816646497968L;
        public int iflag;
    ...
    private static int iter = 0, nfun = 0, point = 0, ispt = 0, iypt = 0, maxfev = 0, info[] = new int[1], bound = 0, npt = 0, cp = 0, i = 0,
     ...
     * scaleCamera optimization", by D. Liu and J. Nocedal, Mathematical Programming B
     * 45 (1989) 503-528.
     *
            ...
            throws ExceptionWithIflag
    {
        ...
        while (true)
        {
            ...
            {
                iflag[0] = -1;
                throw new ExceptionWithIflag(iflag[0], "Line search failed. See documentation of routine mcsrch. Error return of line search: info = "
                        + info[0] + " Possible causes: function or gradient are incorrect, or incorrect tolerances.");
            }
    ...
    public static double ddot(int n, double[] dx, int ix0, int incx, double[] dy, int iy0, int incy)
    {
        ...
        for (i = mp1; i <= n; i += 5)
        {
            dtemp = dtemp + dx[ix0 + i - 1] * dy[iy0 + i - 1] + dx[ix0 + i + 1 - 1] * dy[iy0 + i + 1 - 1] + dx[ix0 + i + 2 - 1] * dy[iy0 + i + 2 - 1]
                    + dx[ix0 + i + 3 - 1] * dy[iy0 + i + 3 - 1] + dx[ix0 + i + 4 - 1] * dy[iy0 + i + 4 - 1];
        }

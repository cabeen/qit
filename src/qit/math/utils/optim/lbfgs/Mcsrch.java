...
public class Mcsrch
{
	...
	public static void mcsrch ( int n , double[] x , double f , double[] g , double[] s , int is0 , double[] stp , double ftol , double xtol , int maxfev , int[] info , int[] nfev , double[] wa )
	{
		...
		{
			infoc[0] = 1;
			if ( n <= 0 || stp[0] <= 0 || ftol < 0 || qit.math.utils.optim.lbfgs.LBFGS.gtol < 0 || xtol < 0 || qit.math.utils.optim.lbfgs.LBFGS.stpmin < 0 || qit.math.utils.optim.lbfgs.LBFGS.stpmax < qit.math.utils.optim.lbfgs.LBFGS.stpmin || maxfev <= 0 )
            {
                return;
            }
			...
			finit = f;
			dgtest = ftol*dginit;
			width = qit.math.utils.optim.lbfgs.LBFGS.stpmax - qit.math.utils.optim.lbfgs.LBFGS.stpmin;
			width1 = width/p5;
		...
		while ( true )
		{
			if ( info[0] != -1 )
			{
				// Set the minima and maxima steps to correspond
				...
				stp[0] = Math.max ( stp[0] , qit.math.utils.optim.lbfgs.LBFGS.stpmin );
				stp[0] = Math.min ( stp[0] , qit.math.utils.optim.lbfgs.LBFGS.stpmax );
			...
			if ( stp[0] == qit.math.utils.optim.lbfgs.LBFGS.stpmax && f <= ftest1 && dg <= dgtest )
            {
                info[0] = 5;
            }

			if ( stp[0] == qit.math.utils.optim.lbfgs.LBFGS.stpmin && ( f > ftest1 || dg >= dgtest ) )
            {
                info[0] = 4;
            }
			...
			if ( f <= ftest1 && Math.abs ( dg ) <= qit.math.utils.optim.lbfgs.LBFGS.gtol * - dginit )
            {
                info[0] = 1;
            }
			...
			if ( stage1 && f <= ftest1 && dg >= Math.min ( ftol , qit.math.utils.optim.lbfgs.LBFGS.gtol ) * dginit )
            {
                stage1 = false;
            }
	...
	public static void mcstep ( double[] stx , double[] fx , double[] dx , double[] sty , double[] fy , double[] dy , double[] stp , double fp , double dp , boolean[] brackt , double stpmin , double stpmax , int[] info )
	{
		...
		if ( fp > fx[0] )
		{
			...
			// The minima is bracketed. If the cubic step is closer
		...
		else if ( sgnd < 0.0 )
		{
			...
			// opposite sign. The minima is bracketed. If the cubic
		...
		else if ( Math.abs ( dp ) < Math.abs ( dx[0] ) )
		{
			...
			// in the direction of the step or if the minima of the cubic
			// is beyond stp. Otherwise the cubic step is defined to be
			// either stpmin or stpmax. The quadratic (secant) step is also
			// computed and if the minima is bracketed then the the step
		...
		else
		{
			...
			// not decrease. If the minima is not bracketed, the step

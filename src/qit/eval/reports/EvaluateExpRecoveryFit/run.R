...
bn <- "EvaluateExpRecoveryFit"
fn <- "data.csv"
system(sprintf("qit %s --output %s", bn, fn))
df <- read.csv(fn)
df <- df[df$alphaerr < 100,]
df$method <- factor(df$method, c("lls", "wlls", "nlls"))

# Plot the data

for (param in c("totalerr", "sigerr", "alphaerr", "betaerr", "alphadel", "betadel", "alphaest", "betaest"))
{
  df$value <- df[,param]

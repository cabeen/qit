...
@ModuleDescription("This program evaluates model fitting of exponential recovery models with a simulation")
@ModuleUnlisted
@ModuleAuthor("Ryan Cabeen")
public class EvaluateExpRecoveryFit implements Module
{
    @ModuleParameter
    ...
    @Override
    public EvaluateExpRecoveryFit run()
    {
        ...
        VectFunction synther = ExpRecovery.synth(varying);
        Vect signal = synther.apply(model.getEncoding());

        Map<String, VectFunction> fitters = Maps.newLinkedHashMap();
        fitters.put(FitExpRecoveryNLLS.NLLS, new FitExpRecoveryNLLS().withVarying(varying).get());
        ...
        for (String ntoken : this.noise.split(","))
        {
            ...
            for (int i = 0; i < this.samples; i++)
            {
                ...
                {
                    ExpRecovery fitted = new ExpRecovery(fitters.get(method).apply(noised));

...
"""  fit a noddi model volume using the package at http://mig.cs.ucl.ac.uk/index.php?n=Tutorial.NODDImatlab.  this requires scans with b=0."""
...
def main():
    ...
    parser.add_option("--output", metavar="<fn>", \
        help="specify an output noddi volume (required)")
    ...
    parser.add_option("--exvivo", \
        help="use the model for ex-vivo (include an isotropic dot)", action="store_true")
    ...
    dwi_fn = join(tmp_dir, "dwi.nii")
    mask_fn = join(tmp_dir, "mask.nii")
    bvecs_fn = join(tmp_dir, "grads.bvec")
    bvals_fn = join(tmp_dir, "grads.bval")
    ...
    bvecs.transpose().write(bvecs_fn)
    grads.getBvals().transpose().write(bvals_fn)

    Logging.info("preparing script")
    root = Global.getRoot()
    runm = "run.m" 
    logm = "log.txt"
    noddim = join(root, "lib", "matlab", "noddi.m")

    if opts.exvivo:
      noddim = join(root, "lib", "matlab", "noddi_exvivo.m")

    base = join(root, "lib", "matlab", "noddi")

    paths = []
    paths.append(join(base, "fitting"))
    paths.append(join(base, "models"))
    paths.append(join(base, "models", "watson"))
    paths.append(join(base, "ParforProgMonv2"))
    paths.append(join(base, "ParforProgMonv2", "java"))
    paths.append("/Users/cabeen/share/niftimatlib-1.2/matlab")

    sf = open(script_fn, "w")
    sf.write('#! /usr/bin/env bash\n')
    sf.write('cd %s\n' % tmp_dir)
    sf.write('mkdir models.noddi\n')
    ...
    baser.mask = mask
    baser.run().mean.write(join(tmp_dir, "models.noddi", "noddi_baseline.nii.gz"))
    ...
    maps.append("kappa")
    maps.append("odi")
    maps.append("fibredirs_xvec")
    maps.append("fibredirs_yvec")
    maps.append("fibredirs_zvec")

    Logging.info("gzipping volumes")
    for m in maps:
      fn = join(tmp_dir, "models.noddi", "noddi_%s.nii" % m)
      if exists(fn):
        if call(["gzip", fn]):
          print("failed to zip %s, skipping" % fn)

    move(join(tmp_dir, "models.noddi"), opts.output) 

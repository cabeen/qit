...
public class VolumeFibersNoise implements Module
{
    @ModuleInput
    ...
    @ModuleParameter
    @ModuleOptional
    @ModuleDescription("the sigma of the orientation noise")
    public Double line = null;
    
    @ModuleParameter
    @ModuleOptional
    @ModuleDescription("the sigma of the volume fraction noise")
    public Double frac = null;

    @ModuleOutput
    @ModuleDescription("output noisy fibers volume")
    public Volume output;

    public VolumeFibersNoise run()
    {
        ...
        for (Sample sample : out.getSampling())
        {
            ...
            {
                Vect linev = fibers.getLine(i);
                double fracv = fibers.getFrac(i);

                if (this.line != null)
                {
                    for (int j = 0; j < linev.size(); j++)
                    {
                        double value = linev.get(j);
                        value += Global.RANDOM.nextGaussian() * this.line;
                        linev.set(j, value);
                    }

                    linev.normalizeEquals();
                }

                if (this.frac != null)
                {
                    this.frac += Global.RANDOM.nextGaussian() * this.frac;
                }

                fibers.setFrac(i, fracv);
                fibers.setLine(i, linev);
            }

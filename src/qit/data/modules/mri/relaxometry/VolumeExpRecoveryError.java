...
@ModuleDescription("Synthesize a volume from exp recovery parameters")
@ModuleAuthor("Ryan Cabeen")
public class VolumeExpRecoveryError implements Module
{
    ...
    @ModuleInput
    @ModuleDescription("input exp recovery sample volume")
    public Volume input;
    ...
    @ModuleInput
    @ModuleDescription("the exponential recovery model parameter volume")
    public Volume model;
    ...
    @Override
    public VolumeExpRecoveryError run()
    {
        final VectFunction synther = ExpRecovery.synth(this.varying.flatten());
        BinaryVectFunction error = new BinaryVectFunction()
        {
            ...
            {
                ExpRecovery model = new ExpRecovery(right);
                ...
                switch(VolumeExpRecoveryError.this.method)
                {
                    case ME:
                        output.set(0, me);
                        return;
                    ...
                    case NRMSE:
                        ...
                        return;
                    default:
                        Logging.error("invalid method: " + VolumeExpRecoveryError.this.method);
                }
            }
        }.init(this.varying.size(), new ExpRecovery().getEncodingSize(), 1);

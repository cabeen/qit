...
public class MaskSetTable implements Module
{
    @ModuleInput
    ...
    @Override
    public MaskSetTable run()
    {
        Table mtable = this.table;

        if (this.lookup != null)
        {
            Logging.info("merging table");
            TableMerge merger = new TableMerge();
            merger.left = mtable;
            merger.right = this.lookup;
            merger.field = this.merge;
            mtable = merger.run().output;
        }

        Map<Integer,Double> lut = TableUtils.createIntegerDoubleLookup(mtable, this.index, this.value);
        ...
        Volume volume = this.reference.protoVolume();

        for (Sample sample : this.reference.getSampling())
        {
            int label = this.reference.get(sample);

            if (lut.containsKey(label))
            {
                volume.set(sample, 0, lut.get(label));
            }
            else if (this.missing != null)
            {
                volume.set(sample, 0, this.missing);
            }
            else
            {
                Logging.error("unknown label: " + label);
            }
        }

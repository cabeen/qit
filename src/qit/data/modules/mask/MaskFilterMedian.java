...
@ModuleDescription("Perform median filtering of a mask.")
@ModuleAuthor("Ryan Cabeen")
public class MaskFilterMedian implements Module
{
    ...
    @Override
    public MaskFilterMedian run()
    {
        VolumeFilterMedian run = new VolumeFilterMedian();
        run.input = this.input.copyVolume();
        run.window = this.window;
        run.mask = this.mask;
        run.slice = this.slice;
        run.num = this.num;
        run.threads = this.threads;

        this.output = MaskSource.create(run.run().output);
        return this;
    }

    public static Mask apply(Mask mask)
    {
        MaskFilterMedian runner = new MaskFilterMedian();
        runner.input = mask;

...
@ModuleDescription("Compute a volumetric mask of curves.  Any voxel that intersects the curves will be included")
@ModuleAuthor("Ryan Cabeen")
public class CurvesMask implements Module
{
    @ModuleInput
    ...
    public CurvesMask run()
    {
        ...
        Volume density = VolumeUtils.density(sampling, this.input);
        VolumeThreshold module = new VolumeThreshold();
        module.input = density;
        module.threshold = this.thresh;
        this.output = module.run().output;

        return this;
    }

    public static Mask apply(Curves curves, Volume ref)
    {
        return new CurvesMask()
        {{
            this.input = curves;
            this.refvolume = ref;
        }}.run().output;

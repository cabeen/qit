...
import qit.data.utils.mesh.MeshCropSolid;
...
@ModuleDescription("Crop a mesh")
@ModuleAuthor("Ryan Cabeen")
public class MeshCrop implements Module
{
    @ModuleInput
    ...
    @ModuleInput
    @ModuleOptional
    @ModuleDescription("remove vertices inside the given solids")
    public Solids solids;

    @ModuleParameter
    @ModuleOptional
    @ModuleDescription("remove vertices that are selected with a non-zero attribute")
    ...
    public MeshCrop run()
    {
        ...
        if (this.solids != null)
        {
            mesh = crop(mesh, this.solids, this.invert);
        }

        if (this.selection != null)
        {
            mesh = crop(mesh, this.selection, this.invert);
        }
    ...
    public static Mesh crop(Mesh input, Solids solid, boolean invert)
    {
        Global.assume(input != null, "input mesh is required");
        Global.assume(solid != null, "selector is required");
    ...
    public static Mesh crop(Mesh input, String label, boolean invert)
    {
        Global.assume(input != null, "input mesh is required");

        if (label == null || !input.vattr.has(label))
        {
            return input;
        }
        ...
        for (Face face : input.graph.faces())
        {
            ...
            Vertex b = face.getB();
            Vertex c = face.getC();
            boolean la = MathUtils.round(input.vattr.get(a, label).get(0)) > 0;
            boolean lb = MathUtils.round(input.vattr.get(b, label).get(0)) > 0;
            boolean lc = MathUtils.round(input.vattr.get(c, label).get(0)) > 0;

            boolean sa = la ^ invert;
            boolean sb = lb ^ invert;
            boolean sc = lc ^ invert;
            int count = (sa ? 1 : 0) + (sb ? 1 : 0) + (sc ? 1 : 0);

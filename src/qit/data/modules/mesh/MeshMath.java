...
public class MeshMath implements Module
{
    @ModuleInput
    ...
    public MeshMath run()
    {
        ...
        for (String field : mesh.vattr.attrs())
        {
        ...
        for (Vertex v : mesh.vattr)
        {
            ...
            {
                e.with(field, mesh.vattr.get(v, field));
            }

            e.with("id", VectSource.create1D(v.id()));

            Vect r = e.eval();

            if (!mesh.vattr.has(this.result))
            {
                mesh.vattr.add(this.result, r.proto());
            }

            mesh.vattr.set(v, this.result, r);
        }

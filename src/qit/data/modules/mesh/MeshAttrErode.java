...
@ModuleDescription("Erode a mesh selection")
@ModuleAuthor("Ryan Cabeen")
public class MeshAttrErode implements Module
{
    @ModuleInput
    ...
    public MeshAttrErode run()
    {
        ...
        {
            Logging.info("started mesh selection erosion");
            for (int i = 0; i < this.num; i++)
            {
                Set<Vertex> changes = Sets.newHashSet();

                Logging.info("started erosion iteration " + i);
                ...
                for (Vertex centerVertex : changes)
                {
                    mesh.vattr.set(centerVertex, outputs[a], VectSource.create1D(0));
                }
            }
            Logging.info("finished mesh selection erosion");
        }

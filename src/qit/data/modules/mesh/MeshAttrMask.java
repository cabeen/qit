...
@ModuleDescription("Apply a mask to an attribute")
@ModuleAuthor("Ryan Cabeen")
public class MeshAttrMask implements Module
{
    @ModuleInput
    ...
    @ModuleParameter
    @ModuleDescription("the mask attribute")
    public String mask = Mesh.MASK;
    ...
    public MeshAttrMask run()
    {
        ...
        for (int a = 0; a < inputs.length; a++)
        {
            ...
            {
                double mask = MathUtils.round(mesh.vattr.get(centerVertex, this.mask).get(0)) > 0 ? 1.0 : 0;
                Vect value = mesh.vattr.get(centerVertex, inputs[a]);
                mesh.vattr.set(centerVertex, outputs[a], value.times(mask));
            }
        }

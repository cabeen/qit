...
public class MeshSetVects implements Module
{
    @ModuleInput
    ...
    public MeshSetVects run()
    {
        Mesh mesh = this.inplace ? this.mesh : this.mesh.copy();

        Vects values = this.vects;
        if (this.vects.size() != this.mesh.vattr.size() && this.vects.getDim() == this.mesh.vattr.size())
        {
            values = VectsUtils.transpose(values);
        }

        Global.assume(values.size() == this.mesh.vattr.size(), "invalid number of vects: " + vects.size());
        mesh.vattr.add(this.name, VectSource.createND(this.vects.getDim()));
        int idx = 0;
        for (Vertex v : mesh.vattr)
        {
            mesh.vattr.set(v, this.name, this.vects.get(idx));
            idx += 1;
        }

...
@ModuleDescription("Evaluate an expression at each vertex of a mesh")
@ModuleAuthor("Ryan Cabeen")
public class MeshAttrMath implements Module
{
    @ModuleInput
    ...
    @ModuleParameter
    @ModuleDescription("the attribute for the result")
    ...
    public MeshAttrMath run()
    {
        ...
        for (Vertex vert : mesh.vattr)
        {
            ...
            {
                e.with(name, mesh.vattr.get(vert, name));
            }

            e.with("id", VectSource.create1D(vert.id()));

            Vect value = e.eval();

            mesh.vattr.set(vert, this.result, value);
        }

        if (this.result.equals(Mesh.COORD))
        {

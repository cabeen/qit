...
@ModuleDescription("Compute the convex hull of a mesh")
...
public class MeshHull implements Module
{
    @ModuleInput
    ...
    @ModuleOutput
    @ModuleDescription("output hull mesh")
    public Mesh output;

    @Override
    public MeshHull run()
    {
        this.output = hull(input);
        return this;
    }

    public static Mesh hull(Mesh mesh)
    {
        Map<Vertex, Vect> lut = Maps.newHashMap();
        for (Vertex vert : mesh.vattr)
        {
            lut.put(vert, mesh.vattr.get(vert, Mesh.COORD));
        }

        Mesh out = VectsHull.hull(lut);

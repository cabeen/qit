...
@ModuleDescription("Which the intensities of a volume")
@ModuleAuthor("Ryan Cabeen")
public class VolumeSubset implements Module
{
    @ModuleInput
    ...
    public VolumeSubset run()
    {
        List<Integer> which = Lists.newArrayList();
        for (String token : this.which.split("[,\\s]+"))
        {
            which.add(Integer.valueOf(token));
        }

        this.output = VolumeUtils.subvolumes(this.input, which);

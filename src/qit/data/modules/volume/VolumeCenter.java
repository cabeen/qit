...
@ModuleDescription("Voxel sizes and position of a volume")
@ModuleAuthor("Ryan Cabeen")
public class VolumeCenter implements Module
{
    @ModuleInput
    ...
    @ModuleParameter
    @ModuleDescription("input voxel dimension scaling factor")
    ...
    public VolumeCenter run()
    {
        ...
        if (this.factor != null)
        {
            delta.timesEquals(this.factor);
        }

        double sx = -0.5 * num.getI() * delta.getX();
        double sy = -0.5 * num.getJ() * delta.getY();
        double sz = -0.5 * num.getK() * delta.getZ();

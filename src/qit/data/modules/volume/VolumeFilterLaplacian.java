...
@ModuleDescription("Filter a volume using a Laplacian kernel")
@ModuleAuthor("Ryan Cabeen")
public class VolumeFilterLaplacian implements Module
{
    @ModuleInput
    ...
    public VolumeFilterLaplacian run()
    {
        final Volume volume = this.input;
        ...
        {
            mysig = 0.5 * ref.deltaMax();
            Logging.info("using voxel-defined smoothing bandwidth: " + mysig);
        }

        Function<Volume, Volume> applyFilter = (filter) ->
        {
            ...
            filterer.withPass(this.preserve);
            ...
            return filterer.withInput(volume).withMask(this.mask).run().getOutput();
        };


        Volume fx = applyFilter.apply(VolumeSource.laplacian(ref, vnum, 1, 1, mysig, this.amp));
        Volume fy = applyFilter.apply(VolumeSource.laplacian(ref, 1, vnum, 1, mysig, this.amp));
        Volume fz = applyFilter.apply(VolumeSource.laplacian(ref, 1, 1, vnum, mysig, this.amp));

        Volume out = VolumeUtils.lincomb(1.0, fx, 1.0, VolumeUtils.lincomb(1.0, fy, 1.0, fz));

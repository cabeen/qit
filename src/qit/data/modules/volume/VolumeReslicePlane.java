...
@ModuleDescription("Reslice a volume by a given plane (contained in a solids object)")
@ModuleAuthor("Ryan Cabeen")
public class VolumeReslicePlane implements Module
{
    private enum VolumeResliceMode
    {
        I, J, K
    }
    ...
    @ModuleInput
    @ModuleDescription("input solids (should contain a plane)")
    private Solids plane;
    ...
    public VolumeReslicePlane run()
    {
        Global.assume(this.plane.numPlanes() > 0, "no plane was found");
        Plane plane = this.plane.getPlane(0);

        Vect axisDir = plane.normal();

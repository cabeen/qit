...
import qit.data.modules.mri.spharm.VolumeSpharmODF;
...
public class SpharmField extends StreamlineField
{
    private VolumeKernelModelEstimator estimator;
    private Vects points;
    private VectFunction odfer;

    public SpharmField(VolumeKernelModelEstimator myestimator, Vects mypoints, int mydim)
    {
        this.estimator = myestimator;
        this.points = mypoints.copy();
        this.odfer = VolumeSpharmODF.factory(mypoints, mydim).get();
    }
    ...
    public List<StreamSample> getLines(Vect pos, Vect ref)
    {
        ...
        Vect odf = this.odfer.apply(this.estimator.estimate(pos, ref));

        for (int idx = 0; idx < this.points.size(); idx++)
        {
            lines.add(new PointSample(pos, this.points.get(idx).copy(), odf.get(idx)));
        }
    ...
    public class PointSample extends StreamSample
    {
        double amp;

        public PointSample(Vect pos, Vect orient, double amp)
        {

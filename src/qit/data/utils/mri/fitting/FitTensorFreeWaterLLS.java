...
public class FitTensorFreeWaterLLS implements Supplier<VectFunction>
{
    ...
    {
        final VectFunction fitter = new FitTensorLLS(){{
            this.gradients = FitTensorFreeWaterLLS.this.gradients;
            this.weighted = FitTensorFreeWaterLLS.this.weighted;
            this.baseline = true;
            this.clamp = 0d;
        }}.get();
        ...
        return new VectFunction()
        {
            ...
            {
                double s0 = ModelUtils.baselineStats(FitTensorFreeWaterLLS.this.gradients, input).mean;

                double lowFrac = 0.0;
                double highFrac = 1.0;

                double bestCost = Double.MAX_VALUE;
                Tensor bestModel = null;
                for (int l = 0; l < schedule.length; l++)
                {
                    ...
                    for (int i = 0; i < num; i++)
                    {
                        ...
                        if (!MathUtils.zero(invfrac))
                        {
                            Vect sig = input.proto();
                            for (int idx = 0; idx < input.size(); idx++)
                            {
                                double bval = FitTensorFreeWaterLLS.this.gradients.getBval(idx);
                                double expiso = Math.exp(-bval * Tensor.FREE_DIFF);
                            ...
                            model = new Tensor(fitter.apply(sig));
                            model.setFreeWater(frac);
                        }

                        Vect pred = synther.apply(model.getEncoding());

                        double testCost = ModelUtils.cost(FitTensorFreeWaterLLS.this.cost, gradients, input, pred);
                    ...
                    double reduce = FitTensorFreeWaterLLS.this.scale / (num - 1);
                ...
                double p = FitTensorFreeWaterLLS.this.prior;

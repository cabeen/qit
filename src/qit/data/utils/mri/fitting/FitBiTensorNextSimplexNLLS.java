...
public class FitBiTensorNextSimplexNLLS implements Supplier<VectFunction>
{
    ...
    public VectFunction createBiTensor()
    {
        ...
        return new VectFunction()
        {
            public void apply(final Vect input, Vect output)
            {
                if (FitBiTensorNextSimplexNLLS.this.verbose)
                {
                ...
                if (FitBiTensorNextSimplexNLLS.this.verbose)
                {
                    Logging.info("starting optimization with " + FitBiTensorNextSimplexNLLS.this.method.name());
                }
                ...
                for (BiTensor initModel : initModels)
                {
                    ...
                    double initError = ModelUtils.cost(FitBiTensorNextSimplexNLLS.this.cost, gradients, input, synther.apply(initModel.getEncoding()));
                    ...
                    double fitError = ModelUtils.cost(FitBiTensorNextSimplexNLLS.this.cost, gradients, input, synther.apply(fitModel.getEncoding()));
    ...
    private BiTensor optimize(BiTensor initModel, VectFunction synther, Parameterizer parameterizer, Vect sig)
    {
        ...
        Calcfc func = new Calcfc()
        {
            ...
            public double Compute(int n, int m, double[] x, double[] con)
            {
                ...
                double cost = ModelUtils.cost(FitBiTensorNextSimplexNLLS.this.cost, gradients, sig, pred);
    ...
    private class Anisotropic extends Parameterizer
    {
        ...
        Vect param(BiTensor tensor)
        {
            ...
            out.set(0,  frac * FitBiTensorNextSimplexNLLS.this.scaleFrac);
            out.set(1,  tissue.get(0, 0) * FitBiTensorNextSimplexNLLS.this.scaleTissue);
            out.set(2,  tissue.get(1, 1) * FitBiTensorNextSimplexNLLS.this.scaleTissue);
            out.set(3,  tissue.get(2, 2) * FitBiTensorNextSimplexNLLS.this.scaleTissue);
            out.set(4,  tissue.get(1, 0) * FitBiTensorNextSimplexNLLS.this.scaleTissue);
            out.set(5,  tissue.get(2, 0) * FitBiTensorNextSimplexNLLS.this.scaleTissue);
            out.set(6,  tissue.get(2, 1) * FitBiTensorNextSimplexNLLS.this.scaleTissue);
            out.set(7,  fluid.get(0, 0) * FitBiTensorNextSimplexNLLS.this.scaleFluid);
            out.set(8,  fluid.get(1, 1) * FitBiTensorNextSimplexNLLS.this.scaleFluid);
            out.set(9,  fluid.get(2, 2) * FitBiTensorNextSimplexNLLS.this.scaleFluid);
            out.set(10, fluid.get(1, 0) * FitBiTensorNextSimplexNLLS.this.scaleFluid);
            out.set(11, fluid.get(2, 0) * FitBiTensorNextSimplexNLLS.this.scaleFluid);
            out.set(12, fluid.get(2, 1) * FitBiTensorNextSimplexNLLS.this.scaleFluid);
        ...
        BiTensor model(BiTensor init, Vect param)
        {
            ...
            double frac = param.get(0) / FitBiTensorNextSimplexNLLS.this.scaleFrac;

            Matrix tC = new Matrix(3, 3);
            tC.set(0, 0, param.get(1) / FitBiTensorNextSimplexNLLS.this.scaleTissue);
            tC.set(1, 1, param.get(2) / FitBiTensorNextSimplexNLLS.this.scaleTissue);
            tC.set(2, 2, param.get(3) / FitBiTensorNextSimplexNLLS.this.scaleTissue);
            tC.set(1, 0, param.get(4) / FitBiTensorNextSimplexNLLS.this.scaleTissue);
            tC.set(2, 0, param.get(5) / FitBiTensorNextSimplexNLLS.this.scaleTissue);
            tC.set(2, 1, param.get(6) / FitBiTensorNextSimplexNLLS.this.scaleTissue);
            ...
            fC.set(0, 0, param.get(7) / FitBiTensorNextSimplexNLLS.this.scaleFluid);
            fC.set(1, 1, param.get(8) / FitBiTensorNextSimplexNLLS.this.scaleFluid);
            fC.set(2, 2, param.get(9) / FitBiTensorNextSimplexNLLS.this.scaleFluid);
            fC.set(1, 0, param.get(10) / FitBiTensorNextSimplexNLLS.this.scaleFluid);
            fC.set(2, 0, param.get(11) / FitBiTensorNextSimplexNLLS.this.scaleFluid);
            fC.set(2, 1, param.get(12) / FitBiTensorNextSimplexNLLS.this.scaleFluid);
        ...
        {
            con[0] = x[0];
            con[1] =  FitBiTensorNextSimplexNLLS.this.scaleFrac - x[0];
        }
    }
    ...
    private class Zeppelin extends Parameterizer
    {
        ...
        Vect param(BiTensor tensor)
        {
            ...
            out.set(0, frac * FitBiTensorNextSimplexNLLS.this.scaleFrac);
            out.set(1, wad * FitBiTensorNextSimplexNLLS.this.scaleFluid);
            out.set(2, wrd * FitBiTensorNextSimplexNLLS.this.scaleFluid);
            out.set(3, wtheta * FitBiTensorNextSimplexNLLS.this.scaleAngle);
            out.set(4, wphi * FitBiTensorNextSimplexNLLS.this.scaleAngle);
            out.set(5, tissue.get(0, 0) * FitBiTensorNextSimplexNLLS.this.scaleTissue);
            out.set(6, tissue.get(1, 1) * FitBiTensorNextSimplexNLLS.this.scaleTissue);
            out.set(7, tissue.get(2, 2) * FitBiTensorNextSimplexNLLS.this.scaleTissue);
            out.set(8, tissue.get(0, 1) * FitBiTensorNextSimplexNLLS.this.scaleTissue);
            out.set(9, tissue.get(0, 1) * FitBiTensorNextSimplexNLLS.this.scaleTissue);
            out.set(10, tissue.get(1, 2) * FitBiTensorNextSimplexNLLS.this.scaleTissue);
        ...
        {
            double frac = param.get(0) / FitBiTensorNextSimplexNLLS.this.scaleFrac;
            double wad = param.get(1) / FitBiTensorNextSimplexNLLS.this.scaleFluid;
            double wrd = param.get(2) / FitBiTensorNextSimplexNLLS.this.scaleFluid;
            double wtheta = param.get(3) / FitBiTensorNextSimplexNLLS.this.scaleAngle;
            double wphi = param.get(4) / FitBiTensorNextSimplexNLLS.this.scaleAngle;
            ...
            Matrix tC = new Matrix(3, 3);
            tC.set(0, 0, param.get(5) / FitBiTensorNextSimplexNLLS.this.scaleTissue);
            tC.set(1, 1, param.get(6) / FitBiTensorNextSimplexNLLS.this.scaleTissue);
            tC.set(2, 2, param.get(7) / FitBiTensorNextSimplexNLLS.this.scaleTissue);
            tC.set(0, 1, param.get(8) / FitBiTensorNextSimplexNLLS.this.scaleTissue);
            tC.set(0, 2, param.get(9) / FitBiTensorNextSimplexNLLS.this.scaleTissue);
            tC.set(1, 2, param.get(10) / FitBiTensorNextSimplexNLLS.this.scaleTissue);
            tC.set(1, 0, tC.get(0, 1));
            tC.set(2, 0, tC.get(0, 2));
            tC.set(2, 1, tC.get(1, 2));
            Matrix tD = tC.times(tC.transpose());
        ...
        void constraints(double[] x, double[] con)
        {
            double scale = FitBiTensorNextSimplexNLLS.this.scaleFluid;

            con[0] = x[0];
            con[1] =  FitBiTensorNextSimplexNLLS.this.scaleFrac - x[0];
            con[2] = x[1] - FitBiTensorNextSimplexNLLS.this.fluidDiffMin * scale;
            con[3] = FitBiTensorNextSimplexNLLS.this.fluidDiffMax * scale - x[1];
            con[4] = x[2] - FitBiTensorNextSimplexNLLS.this.fluidDiffMin * scale;
            con[5] = FitBiTensorNextSimplexNLLS.this.fluidDiffMax * scale - x[2];
            con[6] = x[1] - x[2];
        }
    }
    ...
    private class AlignedZeppelin extends Parameterizer
    {
        ...
        Vect param(BiTensor tensor)
        {
            ...
            out.set(1, wad * FitBiTensorNextSimplexNLLS.this.scaleFluid);
            out.set(2, wrd * FitBiTensorNextSimplexNLLS.this.scaleFluid);
            out.set(3, tv1 * FitBiTensorNextSimplexNLLS.this.scaleTissue);
            out.set(4, tv2 * FitBiTensorNextSimplexNLLS.this.scaleTissue);
            out.set(5, tv3 * FitBiTensorNextSimplexNLLS.this.scaleTissue);
        ...
        BiTensor model(BiTensor init, Vect param)
        {
            ...
            double fad = param.get(1) / FitBiTensorNextSimplexNLLS.this.scaleFluid;
            double frd = param.get(2) / FitBiTensorNextSimplexNLLS.this.scaleFluid;

            double tv1 = param.get(3) / FitBiTensorNextSimplexNLLS.this.scaleTissue;
            double tv2 = param.get(4) / FitBiTensorNextSimplexNLLS.this.scaleTissue;
            double tv3 = param.get(5) / FitBiTensorNextSimplexNLLS.this.scaleTissue;
    ...
    private class Isotropic extends Parameterizer
    {
        ...
        Vect param(BiTensor tensor)
        {
            ...
            out.set(0, frac * FitBiTensorNextSimplexNLLS.this.scaleFrac);
            out.set(1, fluid * FitBiTensorNextSimplexNLLS.this.scaleFluid);
            out.set(2, tv1 * FitBiTensorNextSimplexNLLS.this.scaleTissue);
            out.set(3, tv2 * FitBiTensorNextSimplexNLLS.this.scaleTissue);
            out.set(4, tv3 * FitBiTensorNextSimplexNLLS.this.scaleTissue);
            return out;
        }
        ...
        BiTensor model(BiTensor init, Vect param)
        {
            ...
            double frac = param.get(0) / FitBiTensorNextSimplexNLLS.this.scaleFrac;
            double fluid = param.get(1) / FitBiTensorNextSimplexNLLS.this.scaleFluid;

            double tv1 = param.get(2) / FitBiTensorNextSimplexNLLS.this.scaleTissue;
            double tv2 = param.get(3) / FitBiTensorNextSimplexNLLS.this.scaleTissue;
            double tv3 = param.get(4) / FitBiTensorNextSimplexNLLS.this.scaleTissue;
    ...
    private class BothIsotropic extends Parameterizer
    {
        ...
        Vect param(BiTensor tensor)
        {
            ...
            out.set(0, frac * FitBiTensorNextSimplexNLLS.this.scaleFrac);
            out.set(1, fluid * FitBiTensorNextSimplexNLLS.this.scaleFluid);
            out.set(2, tissue * FitBiTensorNextSimplexNLLS.this.scaleTissue);
        ...
        BiTensor model(BiTensor init, Vect param)
        {
            ...
            double frac = param.get(0) / FitBiTensorNextSimplexNLLS.this.scaleFrac;
            double fluid = param.get(1) / FitBiTensorNextSimplexNLLS.this.scaleFluid;
            double tissue = param.get(2) / FitBiTensorNextSimplexNLLS.this.scaleTissue;
        ...
        void constraints(double[] x, double[] con)
        {
            ...
            con[0] = x[0];
            con[1] =  FitBiTensorNextSimplexNLLS.this.scaleFrac - x[0];
            con[2] = x[1] - scale * FitBiTensorNextSimplexNLLS.this.fluidDiffMin;
            con[3] = scale * FitBiTensorNextSimplexNLLS.this.fluidDiffMax - x[1];
            con[4] = x[1];
            con[5] = x[2];
        }
    }

 ...
 *
 */
public abstract class RowTableModel<T> extends AbstractTableModel implements Iterable<T>
{
    ...
    /**
     *  Remove the all rows from the model.
     */
    public void removeRowAll()
    {
        int rcount = this.getRowCount();
        modelData.clear();
        fireTableRowsDeleted(0, rcount - 1);
    }
    ...
    public Iterator<T> iterator()
    {
        return new Iterator<T>()
        {
            ...
            {
                return this.idx < getRowCount();
            }

            public T next()
            {
                T out = getRow(this.idx);
                this.idx += 1;

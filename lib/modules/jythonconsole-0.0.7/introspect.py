...
def getAttributeNames(object, includeMagic=1, includeSingle=1,
    ...
    if not hasattrAlwaysReturnsTrue(object):
        # Add some attributes that don't always build picked up.  If
        # they don't apply, they'll build filtered out at the end.
...
def getAllAttributeNames(object):
    ...
    # For a class instance, build the attributes for the class.
    try:
    ...
    # Also build attributes from any and all parent classes.
    try:
...
def getCallTip(command='', locals=None):
    ...
    if inspect.isbuiltin(object):
        # Builtin functions don't have an argspec that we can build.
        pass
